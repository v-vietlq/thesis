...
class AsymmetricLoss(nn.Module):
    ...
    def __init__(self, args, class_task=None):
        ...
        # Used also to identify multi-task training
        self.multiset_rank = args.multiset_rank
...
class PiecewiseLoss(nn.Module):
    ...
    def forward(self, output1, output2, label1, label2):
        ...
        if Dg < self.ms:
            loss = 1/2 * \
                torch.pow(torch.clamp(torch.abs(Dp-self.ms), min=0.0), 2)
        elif Dg >= self.ms and Dg <= self.md:
            loss = 1/2 * (torch.pow(torch.clamp(self.ms-Dp, min=0.0), 2) +
                          torch.pow(torch.clamp(Dp-self.md, min=0.0), 2))
        elif Dg > self.md:
            loss = 1/2 * torch.pow(torch.clamp(self.md - Dp, min=0.0), 2)
        return loss
